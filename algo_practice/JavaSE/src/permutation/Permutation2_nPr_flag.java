...
public class Permutation2_nPr_flag {
	...
	static boolean[] isSelected;	//원소가 사용됐는지 중복 체크 배열 => default: false
...
//			System.out.println(Arrays.toString(numbers));
		...
		for (int i = 0; i < N; i++) {
			...
			//원소와 동일한 공간에 t,f 만 확인
			//중복검사 - input[i] 번째 원소가 사용 중인지 검사
			if(isSelected[i]) continue;		// 사용 중인 경우 배제
			numbers[depth] = input[i];		// 원소 사용
			isSelected[i] = true;			// 사용한 원소를 선택했다고 표시
			permutation(depth+1);
...
//		input = new int[] {1,2,3};
		...
		R = input.length;
		numbers = new int[R];
		// 원소에 대해 사용 여부를 표시할 배열이므로 원소 수와 동일하게 선언해야 한다.
