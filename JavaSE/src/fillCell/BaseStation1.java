...
public class BaseStation1 {
    ...
    static int AnswerN=0;		// 메소드 밖에 선언했으므로 static 변수로 선언하기
	public static void main(String[] args) throws Exception {
		System.setIn(new FileInputStream("src/fillCell/basestation1.txt"));			// 이클립스의 기준경로 JavaSE
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		int T = Integer.parseInt(br.readLine());
		for(int test_case=1; test_case<=T; test_case++){
			//테스트케이스 마다 결과 변수를 초기해줘야 한다.  그렇지 않으면 이전 값때문에 결과가 달라진다.  
			AnswerN=0;  
			...
			for (int i = 0; i < N; i++) {
				StringTokenizer st = new StringTokenizer(br.readLine(), " ");		// 공백 주면 토크나이징 더 빨리 한다
				for (int j = 0; j < N; j++) {
