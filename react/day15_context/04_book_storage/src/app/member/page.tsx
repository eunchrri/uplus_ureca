...
export default function MemberDetail() {
  ...
  // setId(member?.id); 무한랜더링 => useEffect를 사용해야 함.
  /* 무한 re-rendering
    id state를 "" 값으로 선언한 후 useMemberContext()를 통해 가져온 member 정보로
    id 상태를 변경하려고 하면, member가 변경될 때마다 id 상태가 변경되기 때문에
    무한 랜더링이 발생한다.
  */
  ...
  useEffect(() => {
    if (member) {
      setId(member.id); // 이렇게 해야 무한 랜더링이 발생하지 않음.
    }
  }, [member]);
