...
export default function Bookmark() {
  ...
  const { bookMarkState, clearBookMark } = useBookMark();
  ...
  return (
    <div className={styles.bookList}>
      <div>
        {/* //////////TODO B21. 이벤트 등록하기  */}
        <button className={styles.registerButton} onClick={handleClearBookMark}>
          모두 삭제
        </button>
      </div>
      <table className={styles.table}>
        ...
        <tbody>
          ...
          {bookMarkState.length > 0 ? (
            bookMarkState.map((book) => <BookMarkItem key={book.isbn} book={book}></BookMarkItem>)
          ) : (
            <tr>
